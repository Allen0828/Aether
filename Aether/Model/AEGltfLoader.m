...
- (AEMesh*)loadMeshFromGLTFAtPath:(NSString*)path error:(NSError *__autoreleasing  _Nullable *)error {
    ...
    // FIX: Expand shared vertices into unique vertices per triangle.
    // The original indexed mesh shares vertices between triangles,
    // but our renderer has an issue with vertex reuse in index buffers.
    // Each triangle gets its own 3 unique vertices.
    NSUInteger triCount = idxCount / 3;
    NSUInteger expandedVertexCount = triCount * 3; // = idxCount
    Vertex *expandedVerts = malloc(sizeof(Vertex) * expandedVertexCount);
    uint16_t *expandedIndices = malloc(sizeof(uint16_t) * expandedVertexCount);
    for (NSUInteger t = 0; t < triCount; t++) {
        NSUInteger i0 = indices[t * 3 + 0];
        NSUInteger i1 = indices[t * 3 + 1];
        NSUInteger i2 = indices[t * 3 + 2];
        expandedVerts[t * 3 + 0] = verts[i0];
        expandedVerts[t * 3 + 1] = verts[i1];
        expandedVerts[t * 3 + 2] = verts[i2];
        expandedIndices[t * 3 + 0] = t * 3 + 0;
        expandedIndices[t * 3 + 1] = t * 3 + 1;
        expandedIndices[t * 3 + 2] = t * 3 + 2;
    }
    free(verts);
    verts = expandedVerts;
